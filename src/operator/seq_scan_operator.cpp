...
Result<RowBatch> SeqScanOperator::next()
{
    ...
    RowBatch batch;
    {
        PROFILE_SCOPE("SeqScan: table.slice");
    ...
    if (predicate_) {
        PROFILE_SCOPE("SeqScan: predicate evaluation");
        masks_ = predicate_->evaluateBatch(batch, output_schema_);
    } else {
        PROFILE_SCOPE("SeqScan: create all-true mask");
    ...
    {
        PROFILE_SCOPE("SeqScan: create rowids");
