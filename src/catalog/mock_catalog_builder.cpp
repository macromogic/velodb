...
std::unique_ptr<Schema> MockCatalogBuilder::createUsersSchema() {
    ...
    schema->addColumn(Column("id", std::make_unique<IntegerType>(), false)); // NOT NULL
    schema->addColumn(Column("name", std::make_unique<VarcharType>(100), false));
    schema->addColumn(Column("email", std::make_unique<VarcharType>(255), false));
    schema->addColumn(Column("age", std::make_unique<IntegerType>(), true)); // Nullable
    schema->addColumn(Column("salary", std::make_unique<DoubleType>(), true));
...
std::unique_ptr<Schema> MockCatalogBuilder::createOrdersSchema() {
    ...
    schema->addColumn(Column("order_id", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("user_id", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("product_id", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("quantity", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("price", std::make_unique<DoubleType>(), false));
    schema->addColumn(Column("order_date", std::make_unique<VarcharType>(20), false)); // Simplified date as string
...
std::unique_ptr<Schema> MockCatalogBuilder::createProductsSchema() {
    ...
    schema->addColumn(Column("product_id", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("name", std::make_unique<VarcharType>(200), false));
    schema->addColumn(Column("description", std::make_unique<VarcharType>(1000), true));
    schema->addColumn(Column("price", std::make_unique<DoubleType>(), false));
    schema->addColumn(Column("stock_quantity", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("category", std::make_unique<VarcharType>(50), true));
...
std::unique_ptr<Schema> MockCatalogBuilder::createComplexSchema() {
    ...
    schema->addColumn(Column("id", std::make_unique<IntegerType>(), false));
    schema->addColumn(Column("int_col", std::make_unique<IntegerType>(), true));
    schema->addColumn(Column("double_col", std::make_unique<DoubleType>(), true));
    schema->addColumn(Column("varchar_small", std::make_unique<VarcharType>(50), true));
    schema->addColumn(Column("varchar_large", std::make_unique<VarcharType>(1000), true));
    schema->addColumn(Column("bool_col", std::make_unique<BooleanType>(), true));
    schema->addColumn(Column("timestamp_col", std::make_unique<VarcharType>(30), true)); // Simplified timestamp
...
DataTypeId MockCatalogBuilder::inferTypeFromExpression(const hsql::Expr* expr) {
    if (!expr) return DataTypeId::VARCHAR;
...
DataTypeId MockCatalogBuilder::inferTypeFromLiteral(const hsql::Expr* literal) {
    if (!literal) return DataTypeId::VARCHAR;
...
std::unique_ptr<Schema> MockCatalogBuilder::createGenericSchema(const std::string& table_name, size_t column_count) {
    ...
    for (size_t i = 0; i < column_count; ++i) {
        ...
        DataTypeId type;
        switch (i % 4) {
            case 0: type = DataTypeId::INTEGER; break;
            case 1: type = DataTypeId::VARCHAR; break;
            case 2: type = DataTypeId::DOUBLE; break;
            case 3: type = DataTypeId::BOOLEAN; break;
            default: type = DataTypeId::VARCHAR; break;
        }
        
        auto data_type = DataType::createType(type);
        schema->addColumn(Column(column_name, std::move(data_type), true));
    }
    
    // Add some common columns based on table name
    if (table_name.find("user") != std::string::npos) {
        schema->addColumn(Column("id", DataType::createType(DataTypeId::INTEGER), false));
        schema->addColumn(Column("name", DataType::createType(DataTypeId::VARCHAR), false));
        schema->addColumn(Column("email", DataType::createType(DataTypeId::VARCHAR), true));
    } else if (table_name.find("order") != std::string::npos) {
        schema->addColumn(Column("order_id", DataType::createType(DataTypeId::INTEGER), false));
        schema->addColumn(Column("user_id", DataType::createType(DataTypeId::INTEGER), false));
        schema->addColumn(Column("total", DataType::createType(DataTypeId::DOUBLE), false));
    } else if (table_name.find("product") != std::string::npos) {
        schema->addColumn(Column("product_id", DataType::createType(DataTypeId::INTEGER), false));
        schema->addColumn(Column("name", DataType::createType(DataTypeId::VARCHAR), false));
        schema->addColumn(Column("price", DataType::createType(DataTypeId::DOUBLE), false));
    }
...
std::unique_ptr<Schema> MockCatalogBuilder::createDynamicSchema(const std::string& table_name,
    ...
    for (const auto& column_name : column_names) {
        if (column_name == "id") continue; // Skip duplicate ID
...
void MockCatalogBuilder::createTableFromCommonSchema(Catalog& catalog, const std::string& table_name) {
    ...
    for (const auto& [column_name, type_id] : common_schemas[table_name]) {
        ...
        schema->addColumn(Column(column_name, std::move(data_type), nullable));
    }
