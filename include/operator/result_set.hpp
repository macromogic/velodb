...
public:
    ...
    size_t getRowCount() const { return tuples_.size() + row_ids_.size(); }
    bool isEmpty() const { return tuples_.empty() && row_ids_.empty(); }

    // Iterator support for materialized tuples
    const std::vector<Tuple>& getTuples() const { return tuples_; }

    // For late materialization
    const std::vector<RowId>& getRowIds() const { return row_ids_; }
    const TableBase* getSourceTable() const { return source_table_; }
