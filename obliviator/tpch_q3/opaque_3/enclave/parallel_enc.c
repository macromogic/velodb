...
int ecall_scalable_oblivious_join(char *input_path, size_t len) {
    ...
    for (int i = 0; i < length1; i++) {
        ...
        arr[i].order_key = atoi(strtok(NULL, " "));
        arr[i].adrevenue = atof(strtok(NULL, " "));
        arr[i].discount = atof(strtok(NULL, " "));
        arr[i].order_date = atoi(strtok(NULL, " "));
        arr[i].ship_priority = atof(strtok(NULL, " "));
        strncpy(key_temp, strtok(NULL, "\n"), 19);
