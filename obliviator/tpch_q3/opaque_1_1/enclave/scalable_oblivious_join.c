...
char target_key[8] = {'B', 'U', 'I', 'L', 'D', 'I', 'N', 'G'};

bool if_building(char* key) {
    bool flag = true;
    for (int i = 0; i < 8; i++) {
        flag = flag && (key[i] == target_key[i]);
    }
    return flag;
}
...
void soj_scan_1(void *voidargs) {
    ...
    for (int i = index_start; i < index_end; i++) {
        //cb1[i] = (88 < arr1[i].key);
        arr1[i].key = if_building(arr1[i].true_key) * i + (!if_building(arr1[i].true_key)) * MAX_DUMMY_ORDER;
        res_thread[thread_order] += if_building(arr1[i].true_key);
    }
...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    if (number_threads == 1) {
        for (int i = 0; i < length1; i++) {
            arr[i].key = 1 - if_building(arr[i].true_key);
            length_result += if_building(arr[i].true_key);
        }
    } else {
