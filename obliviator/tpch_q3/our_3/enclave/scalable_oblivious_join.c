...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    for (int i = 0; i < length && i < 20; i++) {
        ...
        char ip2[20];
        int len2_ip;
        sprintf(ip2, "%d", arr[i].order_date);
        len2_ip = my_len(ip2);

        char revenue2[20];
        int len4_revenue;
        sprintf(revenue2, "%f", arr[i].ship_priority);
        len4_revenue = my_len(revenue2);
        ...
        strncpy(char_current, ip2, len2_ip);
        char_current += len2_ip; char_current[0] = ' '; char_current += 1;
        strncpy(char_current, revenue2, len4_revenue);
        char_current += len4_revenue; char_current[0] = ' '; char_current += 1;
        char_current[0] = '\n'; char_current += 1;
    }
    char_current[0] = '\0';
