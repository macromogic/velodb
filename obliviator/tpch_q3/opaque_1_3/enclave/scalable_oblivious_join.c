...
void soj_scan_1(void *voidargs) {
    ...
    for (int i = index_start; i < index_end; i++) {
        //cb1[i] = (88 < arr1[i].key);
        arr1[i].key = (19950315 < arr1[i].true_key) * i + (arr1[i].true_key <= 19950315) * MAX_DUMMY_ORDER;
        res_thread[thread_order] += (19950315 < arr1[i].true_key);
    }
...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    if (number_threads == 1) {
        for (int i = 0; i < length1; i++) {
            arr[i].key = 1 - (19950315 < arr[i].true_key);
            length_result += (19950315 < arr[i].true_key);
        }
    } else {
