...
void soj_scan_1(void *voidargs) {
    ...
    for (int i = index_start; i < index_end; i++) {
        cb1[i] = (19940101 <= arr1[i].date) && (arr1[i].date < 19950101) && (0.05 <= arr1[i].discount) && (arr1[i].discount <= 0.07) && (arr1[i].quantity < 24);
    }
...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    if (number_threads == 1) {
        for (int i = 0; i < length1; i++) {
            control_bit[i] = (19940101 <= arr[i].date) && (arr[i].date < 19950101) && (0.05 <= arr[i].discount) && (arr[i].discount <= 0.07) && (arr[i].quantity < 24);
        }  
    }
    else {
