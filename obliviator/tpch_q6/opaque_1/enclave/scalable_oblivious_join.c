...
void soj_scan_1(void *voidargs) {
    ...
    for (int i = index_start; i < index_end; i++) {
        //cb1[i] = (88 < arr1[i].key);
        the_con = (19940101 <= arr1[i].date) && (arr1[i].date < 19950101) && (0.05 <= arr1[i].discount) && (arr1[i].discount <= 0.07) && (arr1[i].quantity < 24);
        
        arr1[i].key = the_con * i + (!the_con) * MAX_DUMMY_ORDER;
        res_thread[thread_order] += the_con;
    }
...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    if (number_threads == 1) {
        for (int i = 0; i < length1; i++) {
            arr[i].key = 1 - ((19940101 <= arr[i].date) && (arr[i].date < 19950101) && (0.05 <= arr[i].discount) && (arr[i].discount <= 0.07) && (arr[i].quantity < 24));
            length_result += ((19940101 <= arr[i].date) && (arr[i].date < 19950101) && (0.05 <= arr[i].discount) && (arr[i].discount <= 0.07) && (arr[i].quantity < 24));
        }
    } else {
    ...
    for (int i = 0; i < length_result; i++) {
        char sum1[20];
        int sum1_len;
        sprintf(sum1, "%.2f", arr[i].price);
        sum1_len = my_len(sum1);

        char sum2[20];
        int sum2_len;
        sprintf(sum2, "%.2f", arr[i].discount);
        ...
        char_current += sum2_len; char_current[0] = '\n'; char_current += 1;
    }
    char_current[0] = '\0';
