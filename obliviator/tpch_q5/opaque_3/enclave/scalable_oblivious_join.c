...
char asian[4] = {'A', 'S', 'I', 'A'};

bool if_asian(char* r) {
    bool flag = true;
    for (int i = 0; i < 4; i++) {
        flag = flag && (r[i] == asian[i]);
    }
    return flag;
}
...
void soj_scan_1(void *voidargs) {
    ...
    for (int i = index_start; i < index_end; i++) {
        //cb1[i] = (88 < arr1[i].key);
        the_con = (arr1[i].c_nationkey == arr1[i].s_nationkey) && if_asian(arr1[i].region);
...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    if (number_threads == 1) {
        for (int i = 0; i < length1; i++) {
            arr[i].key = 1 - ((arr[i].c_nationkey == arr[i].s_nationkey) && if_asian(arr[i].region));
            length_result += ((arr[i].c_nationkey == arr[i].s_nationkey) && if_asian(arr[i].region));
        }
    } else {
