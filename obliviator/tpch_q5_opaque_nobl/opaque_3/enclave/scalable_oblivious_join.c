...
static bool if_asian(const char *region) {
    static const char asian[4] = {'A', 'S', 'I', 'A'};
...
static int compact_matches(elem_t *arr, int length) {
    ...
    for (int read_idx = 0; read_idx < length; read_idx++) {
        bool keep = (arr[read_idx].c_nationkey == arr[read_idx].s_nationkey) &&
            if_asian(arr[read_idx].region);
