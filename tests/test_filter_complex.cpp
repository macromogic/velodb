...
protected:
    ...
    void createTestTable() {
        ...
        Column id_col("id", std::make_unique<IntegerType>());
        Column score_col("score", std::make_unique<DoubleType>());
        Column category_col("category", std::make_unique<VarcharType>(50));
        Column tag_col("tag", std::make_unique<VarcharType>(20));
        Column active_col("active", std::make_unique<BooleanType>());
        Column priority_col("priority", std::make_unique<IntegerType>());
        
        auto schema = std::make_unique<Schema>();
        schema->addColumn(std::move(id_col));
        schema->addColumn(std::move(score_col));
        schema->addColumn(std::move(category_col));
        schema->addColumn(std::move(tag_col));
        schema->addColumn(std::move(active_col));
        schema->addColumn(std::move(priority_col));
        
        catalog_->createTable("test_data", std::move(schema));
        test_table_ = static_cast<Table*>(catalog_->getTable("test_data"));
    }
...
TEST_F(FilterComplexTest, BoundaryValueEquals) {
    ...
    EXPECT_EQ(result->getRowCount(), 2); // Records 4 and 5
    
    const auto& tuples = result->getTuples();
...
TEST_F(FilterComplexTest, BoundaryValueRange) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 3); // Records 1, 4, 5
...
TEST_F(FilterComplexTest, MinMaxValues) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 2); // Records 7 and 8
...
TEST_F(FilterComplexTest, MultiColumnComplexFilter) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 4); // Records 1, 2, 4, 8
...
TEST_F(FilterComplexTest, ComplexNegationLogic) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 3); // Records 1, 2, 4
...
TEST_F(FilterComplexTest, AllRecordsMatch) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 8); // All records
}
...
TEST_F(FilterComplexTest, ContradictoryConditions) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 0); // Impossible condition
}
...
TEST_F(FilterComplexTest, ComplexProjectionWithFilter) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 3); // Records 1, 4, 8
    EXPECT_EQ(result->getSchema().getColumnCount(), 3); // Only id, category, score
...
TEST_F(FilterComplexTest, SelectiveProjectionComplexFilter) {
    ...
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->getRowCount(), 3); // Records 1, 7, 8
    EXPECT_EQ(result->getSchema().getColumnCount(), 2); // Only tag, active
...
TEST_F(FilterComplexTest, VeryComplexCondition) {
    ...
    EXPECT_GE(result->getRowCount(), 0);
    EXPECT_LE(result->getRowCount(), 8);
}
