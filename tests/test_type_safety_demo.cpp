...
protected:
    void SetUp() override {
        ...
        schema_->addColumn(Column("id", std::make_unique<IntegerType>()));
        schema_->addColumn(Column("name", std::make_unique<VarcharType>(50)));
        schema_->addColumn(Column("price", std::make_unique<DoubleType>()));
        schema_->addColumn(Column("active", std::make_unique<BooleanType>()));
    }
...
TEST_F(TypeSafetyDemoTest, ArithmeticEvaluation) {
    ...
    Value result = expr->evaluate(nullptr, nullptr);
...
TEST_F(TypeSafetyDemoTest, CastEvaluation) {
    ...
    // Evaluate the cast
    Value result = cast_expr->evaluate(nullptr, nullptr);
    EXPECT_FALSE(result.isNull());
...
TEST_F(TypeSafetyDemoTest, ComplexExpression) {
    ...
    Value result = multiply_expr->evaluate(nullptr, nullptr);
    EXPECT_FALSE(result.isNull());
    EXPECT_EQ(result.getTypeId(), DataTypeId::INTEGER);
